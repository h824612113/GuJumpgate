...
- `mixed-mailbox-utils.js`：Outlook / iCloud URL 统一邮箱队列纯工具，负责混合格式解析、精确 URL 白名单与两行续接校验、队列归一化与合并、顺序选择、provider 解析和日志脱敏。
- `mixed-mailbox-runtime.js`：统一邮箱队列运行时状态转换层，负责活动条目解析、每轮状态补丁、成功/失败标记，以及持久配置与会话状态边界。
...
- `background/icloud-url-provider.js`：iCloud URL 取信 provider，负责受控 GET 轮询、最终响应 URL 重定向边界校验、JSON / HTML / 文本响应解析、验证码排除与脱敏错误处理。
...
- `sidepanel/mixed-mailbox-queue-manager.js`：侧边栏统一邮箱队列管理器，负责安全展示、汇总、筛选、导入、启停、已用状态和批量删除操作，不向 DOM 暴露邮箱凭据。
