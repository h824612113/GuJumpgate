...
- `mixed-mailbox-utils.js`：Outlook / iCloud URL 统一邮箱队列纯工具，负责混合格式解析、队列归一化与合并、顺序选择、provider 解析和日志脱敏。
...
- `background/icloud-url-provider.js`：iCloud URL 取信 provider，负责受控 GET 轮询、JSON / HTML / 文本响应解析、验证码排除与脱敏错误处理。
