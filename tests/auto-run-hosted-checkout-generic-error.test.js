...
test('auto run skips to next round when hosted checkout hits generic error and retry is disabled', async () => {
  ...
  const controller = module.createAutoRunController({
    ...
    isHostedCheckoutGenericErrorFailure: (error) => /HOSTED_CHECKOUT_GENERIC_ERROR::/.test(String(error?.message || error || '')),
  ...
  assert.ok(logs.some((message) => message.includes('第 1/2 轮检测到 PayPal Checkout genericError，本轮将直接失败并跳过剩余重试。')));
  assert.ok(logs.some((message) => message.includes('第 1/2 轮因 PayPal Checkout genericError 提前结束，自动流程将继续下一轮。')));
  assert.ok(appendedRecords.some((record) => /HOSTED_CHECKOUT_GENERIC_ERROR::/.test(record.reason)));
});
