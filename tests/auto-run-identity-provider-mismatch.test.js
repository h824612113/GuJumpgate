...
test('auto run skips to next round when signup hits identity provider mismatch', async () => {
  ...
  const controller = module.createAutoRunController({
    ...
    isSignupIdentityProviderMismatchFailure: (error) => /SIGNUP_IDENTITY_PROVIDER_MISMATCH::/.test(String(error?.message || error || '')),
    ...
    runAutoSequenceFromNode: async () => {
      runCount += 1;
      if (runCount === 1) {
        throw new Error('SIGNUP_IDENTITY_PROVIDER_MISMATCH::步骤 2：检测到 identity_provider_mismatch / 身份验证方式冲突，当前轮将结束并进入下一轮。');
      }
    },
    runtime: {
      get: () => ({ ...runtimeState }),
  ...
  assert.ok(logs.some((message) => message.includes('第 1/2 轮触发 identity_provider_mismatch/身份验证方式冲突，本轮将直接失败并跳过剩余重试。')));
  assert.ok(logs.some((message) => message.includes('第 1/2 轮因 identity_provider_mismatch/身份验证方式冲突提前结束，自动流程将继续下一轮。')));
  assert.ok(appendedRecords.some((record) => /SIGNUP_IDENTITY_PROVIDER_MISMATCH::/.test(record.reason)));
  assert.equal(unavailableMarks.length, 1);
  assert.equal(unavailableMarks[0].reason, 'identity_provider_mismatch');
  assert.equal(unavailableMarks[0].reasonLabel, '身份验证方式冲突');
});
