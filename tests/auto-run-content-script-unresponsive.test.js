...
test('auto run skips to next round when auth page content script is unresponsive', async () => {
  ...
  const controller = module.createAutoRunController({
    ...
    isAutoRunNextRoundPageFailure: (error) => /内容脚本\s+\d+(?:\.\d+)?\s*秒内未响应/.test(String(error?.message || error || '')),
    ...
    runAutoSequenceFromNode: async () => {
      runCount += 1;
      if (runCount === 1) {
        throw new Error('认证页 内容脚本 6 秒内未响应，请刷新页面后重试。');
      }
    },
    runtime: {
      get: () => ({ ...runtimeState }),
  ...
  assert.ok(logs.some((message) => message.includes('第 1/2 轮触发认证页通信/跳转异常，本轮将直接失败并跳过剩余重试。')));
  assert.ok(logs.some((message) => message.includes('第 1/2 轮因认证页通信/跳转异常提前结束，自动流程将继续下一轮。')));
  assert.ok(appendedRecords.some((record) => /内容脚本\s+6\s*秒内未响应/.test(record.reason)));
});
