...
test('post-login phone verification ignores expired oauth deadline when auth page is add-phone', async () => {
  const step8Module = loadStep8Module();
  const stateUpdates = [];
  const completionPayloads = [];
  const logMessages = [];
  let phoneFlowCalled = false;
  const currentState = {
    nodeId: 'post-login-phone-verification',
    visibleStep: 8,
    oauthUrl: 'https://auth.openai.com/oauth/authorize?client_id=test',
    oauthFlowDeadlineAt: Date.now() - 1000,
    oauthFlowDeadlineSourceUrl: 'https://auth.openai.com/oauth/authorize?client_id=test',
    phoneVerificationEnabled: true,
  };
  ...
  const executor = step8Module.createStep8Executor({
    ...
    getOAuthFlowStepTimeoutMs: async () => {
      throw new Error('expired oauth deadline should not block add-phone handling');
    },
    getState: async () => currentState,
    getTabId: async () => 123,
    phoneVerificationHelpers: {
      completePhoneVerificationFlow: async (_tabId, pageState, options) => {
        phoneFlowCalled = true;
        assert.equal(pageState.state, 'add_phone_page');
        assert.equal(options.visibleStep, 8);
        return { code: '123456' };
      },
    },
    sendToContentScriptResilient: async (_source, message) => {
  ...
  await executor.executePostLoginPhoneVerification(currentState);

  assert.equal(phoneFlowCalled, true);
  assert.equal(stateUpdates[0].oauthFlowDeadlineAt, null);
  assert.equal(stateUpdates[0].oauthFlowDeadlineSourceUrl, null);
  assert.equal(completionPayloads.length, 1);
  assert.equal(completionPayloads[0].phoneVerification, true);
  assert.ok(logMessages.some((message) => message.includes('清理旧 OAuth 总超时')));
});
