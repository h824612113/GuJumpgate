...
test('auto run skips to next round when step 2 hits signup identity rate limit', async () => {
  ...
  const controller = module.createAutoRunController({
    ...
    isSignupIdentityRateLimitFailure: (error) => /SIGNUP_IDENTITY_RATE_LIMIT::/.test(String(error?.message || error || '')),
    ...
    runAutoSequenceFromNode: async () => {
      runCount += 1;
      if (runCount === 1) {
        throw new Error('SIGNUP_IDENTITY_RATE_LIMIT::步骤 2：提交注册身份后触发 rate_limit_exceeded / 请求过多，当前轮将直接停止。');
      }
    },
    runtime: {
      get: () => ({ ...runtimeState }),
  ...
  assert.ok(logs.some((message) => message.includes('第 1/2 轮触发步骤 2 注册身份限流，本轮将直接失败并跳过剩余重试。')));
  assert.ok(logs.some((message) => message.includes('第 1/2 轮因步骤 2 注册身份限流提前结束，自动流程将继续下一轮。')));
});
