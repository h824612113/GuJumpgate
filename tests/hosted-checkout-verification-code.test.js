...
test('hosted checkout relay missing link is treated as terminal sms pool error', async () => {
  const executor = createExecutorWithPayload({
    success: false,
    error: '中转链接不存在',
  });
  ...
  await assert.rejects(
    () => executor.fetchHostedCheckoutVerificationCodeManually({
      verificationUrl: 'http://example.test/api/text-relay/eca_tr_missing',
    }),
    /HOSTED_CHECKOUT_SMS_POOL_TERMINAL::.*中转链接不存在/
  );
});
