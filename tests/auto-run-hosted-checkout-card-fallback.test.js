...
test('auto run skips to next round when hosted checkout falls back to card branch', async () => {
  ...
  const controller = module.createAutoRunController({
    ...
    runAutoSequenceFromNode: async () => {
      runCount += 1;
      if (runCount === 1) {
        throw new Error('HOSTED_CHECKOUT_CARD_FALLBACK::步骤 6：hosted checkout 落到银行卡分支，未跳转到 PayPal。');
      }
    },
    runtime: {
      get: () => ({ ...runtimeState }),
  ...
  assert.ok(logs.some((message) => message.includes('第 1/2 轮检测到 hosted checkout 连续落到银行卡分支，本轮将直接失败并跳过剩余重试。')));
  assert.ok(logs.some((message) => message.includes('第 1/2 轮因 hosted checkout 银行卡分支提前结束，自动流程将继续下一轮。')));
  assert.ok(appendedRecords.some((record) => /HOSTED_CHECKOUT_CARD_FALLBACK::/.test(record.reason)));
});
