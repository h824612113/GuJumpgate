...
test('step 5 treats identity_provider_mismatch page without retry button as direct next-round failure', async () => {
  const attrs = new Map();
  const bodyText = [
    '身份验证错误',
  ...
  sandbox.reportComplete = () => {
    throw new Error('reportComplete should not be called when identity_provider_mismatch page is shown');
  };
  sandbox.waitForElement = async () => {
    throw new Error('waitForElement should not run before identity_provider_mismatch is detected');
  };
