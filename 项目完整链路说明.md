...
## 2026-05-23 Plus Hosted 非免费试用 fallback

- 主线策略：Plus PayPal hosted checkout 在 `plus-checkout-create`（可见步骤 6）检测到 `PLUS_CHECKOUT_NON_FREE_TRIAL` / 今日应付金额非 0 时，不再把自动运行整轮直接判失败；在未开启 `autoRunRetryNonFreeTrial` 且处于自动运行上下文时，先复用 `autoExportCompletedSessionArtifacts` 导出当前 Free 账号的 CPA / SUB2API / cockpit / 9router 多渠道配置，再把 `plus-checkout-create` 标记为完成，让后续 OAuth / 平台验证链路继续执行。
- 接入点：`background.js` 将 `autoExportCompletedSessionArtifacts` 注入 `background/steps/create-plus-checkout.js`；后者在 hosted checkout catch 分支记录 `plusCheckoutSource=non-free-trial-free-account-fallback`、`plusCheckoutNonFreeTrialFallback=true` 和失败原因后执行导出。
- 边界：如果开启了 `autoRunRetryNonFreeTrial`，仍保持原来的失败/换号或换账号重试策略；如果不在自动运行上下文或导出函数不可用，仍走原失败/停止逻辑。非 hosted `plus-checkout-billing`（可见步骤 7）的同类检测本次未扩大修改，避免误跳过真实支付链。

## 2026-05-23 GPTSession2CPAandSub2API 本地集成

- 根因：`scripts/hotmail_helper.py` 原先把 `GPT_SESSION_EXPORT_PROJECT_DIR` 固定到旧机器路径，导致 `/export-gpt-session-configs` 报 `GPTSession2CPAandSub2API project not found`。
- 修复：helper 现在优先读取环境变量 `GPT_SESSION_EXPORT_PROJECT_DIR`，默认使用项目内 `services/GPTSession2CPAandSub2API`；已从 `https://github.com/gtxx3600/GPTSession2CPAandSub2API.git` 集成到该目录。
- 兼容层：上游仓库是纯前端转换页，没有 helper 预期的 `scripts/export-configs.mjs`，因此在集成目录补了 CLI 兼容入口，复用 `docs/index.html` 里的转换逻辑，按现有 helper 命令生成 `cpa.json`、`sub2api.json`、`cockpit.json`、`9router.json` 和 `manifest.json`。
- 已验证：`python -m py_compile scripts\hotmail_helper.py`、`node services\GPTSession2CPAandSub2API\tests\convert-session.test.js`、直接运行 `node services\GPTSession2CPAandSub2API\scripts\export-configs.mjs ... --targets cpa,sub2api,cockpit,9router`、以及 POST `http://127.0.0.1:17373/export-gpt-session-configs` 均通过；当前 helper 已重启并监听 `127.0.0.1:17373`。
- 后续同步：运行 `powershell -ExecutionPolicy Bypass -File scripts\update-gpt-session-exporter.ps1`，脚本会 `fetch/pull --ff-only` 更新 `services\GPTSession2CPAandSub2API`，并自动保留/恢复本项目所需的 `scripts/export-configs.mjs` 兼容入口，随后执行 `node --check` 与上游 `tests\convert-session.test.js`。
...
- 当前项目主线：Chrome MV3 扩展通过 `manifest.json` 加载 `background.js`、`sidepanel/sidepanel.html` 与多组 `content/` 页面脚本；侧栏保存配置并发送 `AUTO_RUN` / `SCHEDULE_AUTO_RUN` / `RESUME_AUTO_RUN`，后台消息路由进入 `startAutoRunLoop`，再按 `data/step-definitions.js` 解析出的节点顺序执行各 `background/steps/*` 执行器。
- 已确认入口：`start-hotmail-helper.bat` 启动 `scripts/hotmail_helper.py`，默认监听 `127.0.0.1:17373`；`scripts/gpc_sms_helper_macos.py` 默认监听 `127.0.0.1:18767`，用于 GPC GoPay 本地短信 OTP；`services/checkout-converter/app.py` 提供 `GET /healthz` 与 `POST /api/checkout`，用于 Plus checkout 云端转换。
- 已验证本地健康项：`node --test tests\hosted-checkout-verification-code.test.js tests\cloudflare-temp-email-utils.test.js` 通过 9 项测试；`python -m py_compile scripts\hotmail_helper.py scripts\gpc_sms_helper_macos.py services\checkout-converter\app.py` 通过；`hotmail_helper.py` 本地启动后 `OPTIONS /messages` 返回 204。
- 已验证步骤矩阵：普通邮箱主链为 `open-chatgpt > submit-signup-email > fill-password > fetch-signup-code > fill-profile > wait-registration-success > plus-checkout-create > plus-checkout-billing > paypal-approve > plus-checkout-return > oauth-login > fetch-login-code > post-login-phone-verification > confirm-oauth > platform-verify`；Plus PayPal 邮箱链为 `open-chatgpt > submit-signup-email > fill-password > fetch-signup-code > fill-profile > plus-checkout-create > oauth-login > fetch-login-code > confirm-oauth > platform-verify`；本地 CPA JSON 链为 `open-chatgpt > submit-signup-email > fill-password > fetch-signup-code > fill-profile > plus-checkout-create > local-cpa-json-export`。
- 当前阻塞/风险：`phone-sms/providers/registry.js` 声明了 `nexsms`，但当前仓库没有对应 `PhoneSmsNexSmsProvider` 实现，选择该 provider 会报“接码平台模块未加载：nexsms”；`services/checkout-converter` 当前 Python 环境缺少 `fastapi` 与 `uvicorn`，需安装 `services/checkout-converter/requirements.txt` 后才能启动服务；PowerShell 下 `node --test tests\*.test.js` 不展开通配符，应显式传入测试文件或用 PowerShell 先枚举。
- 下一步验证顺序：先补齐 checkout-converter 依赖并测 `/healthz`，再加载扩展进行侧栏配置恢复与 `AUTO_RUN` 消息链路验证，最后按竞赛环境实际配置逐条验证邮箱、接码、支付转换和平台回调分支；`nexsms` 在实现前应从可选路径中熔断或改用 HeroSMS / 5sim。
