...
5. 执行 `runAutoSequenceFromStep`
   ...
   - 步骤 5 启动前会先确认 Step 4 的邮箱验证码页是否真的退出；如果认证页只是短暂成功推定但仍停留在 `/email-verification`，会先等待或自动恢复 `Retry / 重试` 页；超过上限仍未退出时，会抛出专用错误并直接结束当前轮进入下一轮，而不是继续报“未找到姓名输入框”
   - 步骤 2 在提交邮箱/手机号后，如果认证页直接进入 `rate_limit_exceeded / 请求过多` 之类的身份限流错误页，会被识别为专用注册限流错误；自动运行会直接结束当前轮并进入下一轮，不再把它误判为普通“等待页面跳转超时”
...
- 扩展清单入口：[manifest.json](/Users/hanhao/Documents/Gujump-author/manifest.json:1)
- 后台 Service Worker 入口：[background.js](/Users/hanhao/Documents/Gujump-author/background.js:1)
- 侧边栏入口：[sidepanel/sidepanel.html](/Users/hanhao/Documents/Gujump-author/sidepanel/sidepanel.html:1) / [sidepanel/sidepanel.js](/Users/hanhao/Documents/Gujump-author/sidepanel/sidepanel.js:1)
- 步骤定义入口：[data/step-definitions.js](/Users/hanhao/Documents/Gujump-author/data/step-definitions.js:1)
- 步骤注册入口：[background/steps/registry.js](/Users/hanhao/Documents/Gujump-author/background/steps/registry.js:1)
- 自动运行控制入口：[background/auto-run-controller.js](/Users/hanhao/Documents/Gujump-author/background/auto-run-controller.js:1)
- 消息路由入口：[background/message-router.js](/Users/hanhao/Documents/Gujump-author/background/message-router.js:1)

### 自动运行控制链

当前已确认的自动运行控制链为：

1. 侧边栏点击“自动”后，`startAutoRunFromCurrentSettings()` 保存配置并发送 `AUTO_RUN` 或 `SCHEDULE_AUTO_RUN`。
2. `background/message-router.js` 接收消息，执行模式校验、运行态锁定、倒计时计划检查，并调用 `startAutoRunLoop()`。
3. `background/auto-run-controller.js` 创建 `autoRunSessionId`，维护当前轮次、尝试次数、失败原因和轮次摘要。
4. 自动运行每轮通过 `runAutoSequenceFromWorkflowNode()` 从当前节点开始执行。
5. `workflow-engine` 根据 `nodeStatuses` 判断运行中节点、第一个未完成节点、完成态和终止态。
6. `steps/registry` 将步骤 key 映射到真实执行器。
7. 具体步骤执行器分布在 `background/steps/`，页面交互由对应 `content/` 脚本完成。

停止链路由后台 `requestStop()` 收口：取消 pending command，广播停止给 content scripts，拒绝等待器，把 running 节点标记为 `stopped`，并清理当前自动运行 session，避免旧 timer 或旧恢复入口继续拉起流程。
...
- `node --test tests/*.test.js`：83 个测试通过
- `python3 tests/sub2api_error_refresh_helper_snapshot_test.py`：1 个测试通过

本验证不启动 Chrome 真实扩展流程，不访问真实邮箱、支付、接码或平台回调页面。
...
- 默认地址：`http://127.0.0.1:17373`
- 健康检查：`GET /health`
- `POST /messages` 与 `POST /code` 会触发真实邮件服务访问，只能在明确需要跑真实链路时使用。
...
- 默认地址：`http://127.0.0.1:18767`
- 接口：`/health`、`/latest-otp`、`/otp`

### 当前缺口

当前仓库没有统一一键验证入口，`package.json` 也没有 `scripts`。后续要推进全自动化，应优先补齐：

1. 本地纯验证脚本：语法检查、Node 测试、Python 测试、helper 健康检查。
2. 外部服务 mock 层：邮箱、接码、支付、CPA / SUB2API 回调。
3. 受控扩展 E2E：固定扩展实例、固定配置、可重复启动和收集日志。

## 2026-06-03 当前主线校准

### 最高指令文件状态

- 项目级最高指令文件为 [AGENTS.md](/Users/hanhao/Documents/Gujump-author/AGENTS.md:1)。
- 用户原始总控指令全文已归档在 `## 用户原始总控指令归档` 段落，无需重复写入。
...
1. 侧栏保存配置并触发自动运行。
2. 后台消息路由收口命令、校验模式并启动自动运行会话。
3. 自动运行控制器按轮次推进 workflow 节点，记录失败原因、重试与熔断。
4. 步骤注册表把节点映射到 `background/steps/` 执行器。
5. 具体页面交互由 `content/` 内容脚本完成。
6. 成功链路在末端进入 OAuth 回调验证，或直接导出 / 导入 SESSION 到本地 helper、CPA、SUB2API。
...
- `python3 tests/sub2api_error_refresh_helper_snapshot_test.py`：通过。
- `node --test tests/*.test.js`：`82` 个测试里 `80` 通过、`2` 失败。

当前失败用例：

1. [tests/plus-checkout-hosted-paypal-selection.test.js](/Users/hanhao/Documents/Gujump-author/tests/plus-checkout-hosted-paypal-selection.test.js:6)  
   `hosted checkout paypal selector uses hosted checkout specific button lookup`
2. [tests/plus-checkout-hosted-paypal-selection.test.js](/Users/hanhao/Documents/Gujump-author/tests/plus-checkout-hosted-paypal-selection.test.js:35)  
   `hosted checkout page detection includes chatgpt checkout hosted path`

对应现状代码位于 [content/plus-checkout.js](/Users/hanhao/Documents/Gujump-author/content/plus-checkout.js:138) 和 [content/plus-checkout.js](/Users/hanhao/Documents/Gujump-author/content/plus-checkout.js:363)。这说明“Hosted Checkout 的 PayPal 选择器 / 页面识别”与当前测试预期已经发生偏移，现阶段不能再把 Node 基线视为全绿。

### 当前阻塞点

- 文档里的旧基线 `83` 个测试全通过已经过期，主线判断必须改用本节结果。
- 缺少统一一键验证入口，导致当前验证仍需手动分别调用 Node / Python 命令。
- `content/plus-checkout.js` 与 Hosted Checkout 相关测试存在偏移，后续若要推进全自动化，优先级应放在这里。

### 2026-06-03 Step 6 补充：Plus Checkout 401 熔断

- 当 `plus-checkout-create` 命中 `创建 Plus Checkout 失败：HTTP 401` 或等价未授权错误时，不再在当前轮内部回到 `plus-checkout-create` 自重试。
- 当前轮会直接判定失败，跳过本轮剩余重试，并在自动运行有后续轮次时直接进入下一轮。
- 该规则同时作用于：
  - 节点内 Checkout 重建判断，避免第 6 步在同一轮里循环重建
  - 轮次级自动运行熔断判断，确保失败会按“当前轮结束 -> 下一轮”推进
